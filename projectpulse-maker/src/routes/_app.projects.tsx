      ...
      <main className="flex-1 space-y-4 p-6">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {projects.map((p) => (
            <Card
              key={p.id}
              className="group flex flex-col gap-4 p-5 transition hover:border-primary/40 hover:shadow-glow"
            >
              ...
              <div className="flex items-center justify-between border-t border-border pt-3">
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  <span>
                    ...
                    {format(new Date(p.endDate), "MMM d, yy")}
                  </span>
                </div>
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`rounded-md border px-2 py-0.5 text-[10px] font-medium uppercase ${tone}`}>
      {status.replace("_", " ")}
    </span>
  );
}
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Set up a new workspace for tasks, sprints, and incidents.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={form.type}
              onValueChange={(v) => setForm({ ...form, type: v as "KANBAN" })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
