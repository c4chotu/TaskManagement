  ...
  if (!task)
    return (
      <>
        <Topbar title="Task" />
        <main className="p-6">
          <Card className="p-8 text-center text-sm text-muted-foreground">Task not found.</Card>
        </main>
      </>
    );
      ...
      <main className="flex-1 space-y-4 p-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
              ...
              <Card className="border-destructive/30 bg-destructive/5 p-5">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <Field
                    ...
                    value={issue.customerReported ? `Yes · ${issue.customerName ?? ""}` : "No"}
                  />
                  <Field
                    label="Status"
                    value={
                ...
                {issue.customerImpact && (
                  <div className="mt-3 rounded border border-destructive/30 bg-background/40 p-3 text-xs text-muted-foreground">
                    <span className="font-medium text-foreground">Customer impact: </span>
                    {issue.customerImpact}
                  </div>
                )}
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <SlaCountdown
                    label="Response SLA"
                    target={issue.slaTargetResponse}
                    done={issue.acknowledged}
                  />
          ...
          <aside className="space-y-4">
            <Card className="p-4">
              <h4 className="mb-3 text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
                Details
              </h4>
            ...
            <Card className="p-4 space-y-3">
              ...
              {suggestion && suggestion.suggestedAssigneeId && (
                <div className="rounded border border-primary/20 bg-primary/5 p-2 text-xs">
                  <span className="font-semibold text-[10px] uppercase tracking-wide text-primary block">
                    Suggested Assignee
                  </span>
                        ...
                        <>
                          <Avatar className="h-5 w-5">
                            <AvatarFallback className="bg-muted text-[9px]">
                              {su?.name ? su.name.slice(0, 2).toUpperCase() : "??"}
                            </AvatarFallback>
                          </Avatar>
                  ...
                  {!task.assigneeIds.includes(suggestion.suggestedAssigneeId) && (
                    <Button
                      size="sm"
                      className="w-full h-6 text-[10px] mt-2 bg-gradient-primary text-primary-foreground"
              ...
              <Card className="p-4">
                ...
                <div className="space-y-3 max-h-48 overflow-y-auto pr-1 scrollbar-thin">
                  {routingHistory.map((h) => {
                    const prevUser = h.previousAssigneeId
                      ? users.find((u) => u.id === h.previousAssigneeId)
                      : null;
                    ...
                    return (
                      <div key={h.id} className="border-l-2 border-primary/30 pl-2 text-xs">
                        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                          ...
                          <span>
                            {formatDistanceToNow(new Date(h.assignedAt), { addSuffix: true })}
                          </span>
                        </div>
                        <p className="mt-0.5 font-medium">
                          {prevUser ? prevUser.name : "Unassigned"} →{" "}
                          {newUser ? newUser.name : "Unassigned"}
                        </p>
                        {h.reason && (
                          <p className="mt-0.5 text-[10px] text-muted-foreground">{h.reason}</p>
                        )}
                      </div>
                    );
                  })}
