...
import { addMonths, endOfMonth, format, isSameDay, startOfMonth, startOfWeek, endOfWeek, addDays, isSameMonth } from "date-fns";
  ...
  return (
    <>
      ...
      <main className="flex-1 space-y-4 p-6">
        <Card className="p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => setCursor(addMonths(cursor, -1))}><ChevronLeft className="h-4 w-4" /></Button>
          ...
          <div className="grid grid-cols-7 gap-px overflow-hidden rounded-lg border border-border bg-border">
            {["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((d) => (
              <div key={d} className="bg-muted/40 px-2 py-1.5 text-[10px] font-mono uppercase text-muted-foreground">{d}</div>
            ))}
            {days.map((d) => {
              ...
              return (
                ...
                >
                  <div className={`mb-1 text-[10px] font-mono ${isToday ? "text-primary" : "text-muted-foreground"}`}>{format(d, "d")}</div>
                  <div className="space-y-1">
                    {dayTasks.slice(0, 4).map((t) => (
                      <Link
                        key={t.id} to="/tasks/$id" params={{ id: t.id }}
                      ...
                      >
                        {t.taskType === "ISSUE" && <AlertOctagon className="mr-0.5 inline h-2.5 w-2.5" />}
                        {t.title}
                      </Link>
                    ))}
                    {dayTasks.length > 4 && <div className="px-1 text-[10px] text-muted-foreground">+{dayTasks.length - 4} more</div>}
                  </div>
                </div>
              );
            })}
        ...
        <p className="text-xs text-muted-foreground">Tip: drag any task or incident card to a different day to reschedule. Changes are persisted via the REST API.</p>
      </main>
    </>
  );
}
