...
import { AssigneeStack, TaskStatusSelect, TaskAssignPopover } from "@/components/tfp/task-quick-edit";
        ...
        <Card className="overflow-hidden">
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/30 text-left text-[10px] uppercase tracking-wide text-muted-foreground">
              <tr><th className="px-4 py-2">Task</th><th className="px-4 py-2">Project</th><th className="px-4 py-2">Status</th><th className="px-4 py-2">Assignees</th><th className="px-4 py-2">Due</th><th className="px-4 py-2"></th></tr>
            </thead>
            <tbody>
              {filtered.map((t) => {
                ...
                return (
                  <tr key={t.id} className="border-b border-border/50 transition hover:bg-muted/30">
                    <td className="px-4 py-3">
                      <Link to="/tasks/$id" params={{ id: t.id }}>
                        <div className="flex items-center gap-2">
                          ...
                          {t.taskType === "ISSUE" && <Badge variant="outline" className="h-4 border-destructive/30 px-1 text-[10px] text-destructive">Issue</Badge>}
                        </div>
                        <p className="font-medium">{t.title}</p>
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                      {p && <Link to="/projects/$id" params={{ id: p.id }} className="hover:text-primary">{p.name}</Link>}
                    </td>
                    <td className="px-4 py-3">{s ? <TaskStatusSelect task={t} compact /> : null}</td>
                    ...
                    <td className="px-4 py-3 text-right"><TaskAssignPopover task={t} /></td>
                  </tr>
                );
              })}
            </tbody>
